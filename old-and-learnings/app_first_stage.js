...
//for getting one tour by id we add a route parameter to the url by adding a colon before the name of the parameter. This means that we can access the value of the parameter in the req.params object. So if we make a GET request to /api/v1/tours/5 then req.params.id will be 5 (as a string). We can then use this id to find the tour in our data and send it back in the response. We can also make a param optional by wrapping the parameter in curly braces (rather than adding a question mark after it which is the Express 4 way of doing it suggested in the course lecture)
